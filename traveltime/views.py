...
def neighbours(center, time):
	...
	for v in t1:
		...
		locs[s] = (town.longitude, town.latitude)

	return query_points, locs, dest.longitude, dest.latitude
...
def results(request):
	...
	start_longs = [locs[n][0] for n in names]
	start_lats = [locs[n][1] for n in names]
